/**
 * <cmx-date-input> — 日期输入列编辑器（form + grid 双端）
 *
 * 封装 UI5 <ui5-date-picker>，作为 'date' 字段类型编辑器。
 * 首版完成基础能力（格式设置 + 下拉日历选择），范围限制/特殊日历等后续完善。
 *
 * 公开 API：
 *   setField(field)         读 formatPattern/format/minDate/maxDate/placeholder/readonly
 *   setValue(v,{silent}) / getValue()   值为字符串（按 formatPattern 文本，默认 yyyy-MM-dd）
 *   setReadonly(b) / setPlaceholder(t) / focus()
 *   validate() -> true | string
 *
 * @component
 * @fires cmx-value-change - 值变化时触发（bubbles + composed），detail: { value, valid }
 * @attr {string} format - 日期格式串（默认 yyyy-MM-dd）
...
export class CmxDateInput extends HTMLElement {
  ...
  _hasAnyObservedAttr () {
    return CmxDateInput.observedAttributes.some((n) => this.hasAttribute(n))
  }
  ...
  /** 设置只读状态并同步到 picker。 @param {boolean} b 是否只读 @returns {CmxDateInput} this */
  setReadonly (b) { this._readonly = !!b; this._applyToPicker(); return this }
  /** 设置占位文本并同步到 picker。 @param {string} t 占位文本 @returns {CmxDateInput} this */
  ...
  /**
   * 校验当前值（委托 ui5-date-picker 自带格式校验）。空值放过。
   * @returns {true|string} true 表示合法；否则返回错误信息字符串
   */
  validate () {
    const el = this._picker
    // UI5 date-picker 自带格式校验
    if (el && typeof el.isValid === 'function' && this._value && !el.isValid(this._value)) return '日期格式不正确'
    return true
  }
  ...
  /** 首次渲染：创建 shadow DOM 样式 + ui5-date-picker，绑定 change 事件。 */
  _render () {
    const style = document.createElement('style')
    style.textContent = ':host{display:block;width:100%;min-width:0;box-sizing:border-box}' +
      'ui5-date-picker{display:block;width:100%;min-width:0;box-sizing:border-box;--_ui5_input_min_width:0}' +
      /* DatePicker unlike DateTimePicker does not force its inner input root to fill the host. */
      'ui5-date-picker::part(input){display:block;width:100%;min-width:0;box-sizing:border-box}' +
      ':host([data-cmx-fill-host]){height:100%;box-sizing:border-box}' +
      ':host([data-cmx-fill-host]) ui5-date-picker{height:100%;margin:0;vertical-align:top}'
  ...
  /* UI5 ui5-date-picker 内部的日历图标(.inputIcon)固有 align-self:start，在更高的
     单元格编辑器里顶部对齐显得偏上。进入 picker shadow 把它改为 center 垂直居中。
     图标可能晚于 picker 挂载，故重试几帧。 */
...
if (!customElements.get('cmx-date-input')) customElements.define('cmx-date-input', CmxDateInput)
