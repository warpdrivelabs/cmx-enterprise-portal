...
export async function login ({ username, password, deviceType = 'web', deviceId } = {}) {
  ...
  // 同步用户身份到 localStorage（cmx_user_id/cmx_username）：流程待办中心等页面按
  // 「当前用户数字 id」过滤任务（CFG.getUser 约定读这两把 key），此前登录只写 token，
  // 导致按人过滤恒拿 'admin' 用户名兜底、与任务的数字 id assignee 不匹配。
  syncUserIdentity().catch(() => {})
  return data
}

/**
 * 把当前登录用户的数字 id / 用户名同步到 localStorage（cmx_user_id / cmx_username）。
 * fire-and-forget 场景调用；失败静默（待办中心会退回用户名兜底）。
 */
export async function syncUserIdentity () {
  const me = await fetchCurrentUser()
  if (!me) return
  try {
    if (me.user_id) localStorage.setItem('cmx_user_id', String(me.user_id))
    if (me.username) localStorage.setItem('cmx_username', String(me.username))
  } catch {}
}
...
export async function logout () {
  ...
  try {
    ...
    clearTokens()
    try {
      localStorage.removeItem('cmx_user_id')
      localStorage.removeItem('cmx_username')
    } catch {}
  }
}
